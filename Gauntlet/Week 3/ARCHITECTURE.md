...
*Status: draft for Architecture Defense. Full detail (regression harness internals, observability
schema, contract definitions) fills in through MVP per the project schedule.*
...
`agent/`), reusing the framework, model-swap pattern (`ANTHROPIC_MODEL` env var), and Langfuse
observability wiring already proven in Weeks 1-2 rather than introducing a second stack. Logical
separation between agents comes from code-level isolation (each node gets its own system prompt, its own
model client, and a narrow, typed slice of shared state — never the full state object) plus versioned
Pydantic message schemas at each node boundary, not from splitting into microservices; that split isn't
justified at this scale and would multiply the deployment surface for no security benefit. If the cohort's
peer-integration exercise activates later in the week, these Pydantic schemas double as the contract
definitions (published to `/contracts` as JSON Schema) without rework.
...
Orchestrator flags a category as under-covered and worth a more creative attempt — mirroring how real red
teams triage cheap fuzzing before expensive manual effort. Commercial models sometimes refuse
offensive-security framing even for authorized, defensive testing; the mitigation is framing every Red
Team prompt as testing our own system (never a third party) plus a documented fallback to an open-weight
model for the minority of payloads Claude declines, tracked as a build-vs-configure decision, not silently
worked around.
...
of the target (not just a successful attack) trips a circuit breaker and logs for review rather than
continuing.

Traditional tooling isn't replaced wholesale — Garak covers known/static OWASP-LLM-Top-10 techniques
cheaply as a regression-suite baseline layer (it isn't multi-turn or system-specific, so it can't replace
the custom Red Team Agent, only complement it), and OWASP ZAP handles classic protocol-level web
vulnerabilities (auth, injection, SSRF) deterministically, wrapped as an Orchestrator-invokable tool feeding
the same exploit database and report pipeline as the LLM-driven agents.
...
- The inter-agent contracts (`NextTarget`, `AttackSequence`, `JudgeVerdict`, `ExploitRecord`) describe
  agent-to-agent messages, not target payloads — a `JudgeVerdict` looks the same regardless of target.
...
- How an attack is actually delivered — `proxy.php`'s request shape, CSRF token, OAuth2/SMART session.
...
**The mechanism**: a `TargetAdapter` interface —
...
— plus a `target_profile.yaml` per deployment (auth method, sensitive-field list, endpoint list, rate
limits). Everything OpenEMR-specific lives in one `OpenEMRAdapter` implementation and its profile; the
Orchestrator, Red Team, Judge, and Documentation agents only ever call the adapter interface, never
OpenEMR directly. The Exploit DB keys each row by `(target_id, target_version, attack_category)` — not just
`target_version` — so multiple targets' regression histories don't collide if they ever share a deployment,
and a single target's history stays cleanly filterable if exported on its own.
...
2. **Judge never sees Red Team's reasoning** — only the final transcript and the target's observed
   ...
   state. This is the conflict-of-interest fix the assignment calls out by name.
3. **Model tiering by role, not one model for everything** — cheap model for high-volume Red Team mutation,
   expensive model reserved for Judge consistency and Orchestrator-flagged hard targets. Grounded in real
   Week 1-2 cost data, not a guess.
4. **Regression pass requires rubric_version + target_version pinning** — prevents the "passed because the
   model drifted" failure mode the assignment explicitly warns is worse than no test.
5. **Narrow, explicit human gates** — Critical/High report publication only; no auto-remediation ever.
   Everything else is autonomous by design, per the assignment's "without a human in the loop for every
   step" framing.
...
7. **A Target Adapter Layer separates the target-agnostic core from target-specific integration** — only a
   ...
   fields; the four agents, the contracts, the regression harness, and the observability layer never
   reference OpenEMR directly. Rejected: baking target specifics into each agent's own logic, which would
   mean rearchitecting the platform (not just writing one adapter) to point it at a different system.

## Open items before MVP (Stages 2-4)

- `./THREAT_MODEL.md` — full attack surface map across all 6 required categories, prioritized by the
  concrete findings from initial code review (coarse per-patient ACL, client-echoed conversation history,
  document-ingestion injection path).
- `./evals/` — seed attack suite covering ≥3 categories + first working agent prototype against the live
  deployed target.
- Exploit DB schema, observability query set, and full contract definitions in `/contracts`.
- `TargetAdapter` interface + `target_profile.yaml` schema, plus the concrete `OpenEMRAdapter`
  implementation (auth handling, request shaping, PHI-field classification for this target).
