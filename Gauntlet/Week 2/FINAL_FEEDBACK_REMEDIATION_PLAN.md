...
### 10. Judge reproducible; results recorded — **DONE**
...
**Fix:** `run_eval_gate.py` now writes `agent/eval/latest_results.md` on every full (non-`--tier1-only`)
run — a per-rubric pass-rate table plus a per-case table (id, category, PASS/FAIL, which specific rubrics
failed if any). New pure `render_latest_results_md()` function (2 unit tests) builds the markdown from the
same `outcomes`/`rubric_results` data `aggregate_by_rubric()` already consumes, so no new live-run
machinery was needed — `run_golden_set()` just also returns the per-case data it already collected instead
of discarding it. **Verified live**: ran the full golden set twice against the real Anthropic + Voyage APIs
to produce and commit a genuine result (49/50 first representative run, MSD-07 being the one known-flaky
case; the checked-in file reflects this real run's output verbatim, not a synthetic example). Tier 1 suite:
...
| 10 | Judge reproducible; results recorded | 2/3 | **P2 — done, live-verified** | 1–2 |
