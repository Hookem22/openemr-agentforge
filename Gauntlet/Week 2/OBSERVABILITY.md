...
| 7 | **Eval regression** | Any `eval_gate_pass_rate_{rubric}` score drops >5 percentage points from its previous value, or any rubric's score < 80% -- `{rubric}` is one of `schema_valid`, `citation_present`, `factually_consistent`, `safe_refusal`, `no_phi_in_logs` (grader-flagged fix, 2026-07: this was previously keyed by test-case domain category -- citations/refusals/extraction/etc. -- not by the actual rubric booleans; threshold separately corrected from 15 to the spec's 5 points, Final feedback -- the 15-point widening was a fix for the old category-based 10-case denominator, which stopped applying once aggregation became per-rubric across all 50 cases) | Filter scores by name starting with `eval_gate_pass_rate_`, numeric value trend | A live behavioral regression in extraction accuracy, retrieval relevance, citation correctness, refusal safety, or missing-data handling -- the *same* mechanism `agent/eval/run_eval_gate.py` uses for the PR-blocking gate, surfaced as a live alert so a regression introduced *between* pushes (e.g. an Anthropic model version change, a corpus edit) is also caught. `.github/workflows/agent-tier2.yml` runs `python eval/run_eval_gate.py --push-to-langfuse` on a daily schedule (and the plain gate, no Langfuse push, on every PR) to keep this alert's data fresh. | Run `python eval/run_eval_gate.py` locally to get the specific failing case ids and `golden_checks.py` failure detail for the failing rubric, then treat it exactly like a CI gate failure (Section 6). |
