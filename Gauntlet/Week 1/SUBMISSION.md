# SUBMISSION.md — AgentForge: Clinical Co-Pilot (Week 1)

**Week 1 status: complete and submitted.** High-level summary of Week 1 work — see the linked documents
for full detail on any given area. For current/ongoing status (Week 2 and beyond), see `../STATUS.md`.
...
- **Observability**: Langfuse Cloud tracing wired into every LangGraph node — one trace per chat turn, with
  ...
  **PHI redacted in code before it reaches Langfuse Cloud**: every `@observe` decorator disables Langfuse's
  ...
  API: `PHI_AUDIT.md`. Full self-hosting remains a documented, deferred future option
  (`LANGFUSE_SELFHOST.md`), not required given this redaction.
...
- **Load/stress testing (10 & 50 concurrent users)**: done — `LOADTEST.md`. Real run against the deployed
...
- **Langfuse Cloud PHI compliance**: done via code-level redaction — `PHI_AUDIT.md`. Full self-hosting
  remains intentionally deferred (`LANGFUSE_SELFHOST.md`), since it's no longer required for compliance,
  just a stronger infrastructure-level guarantee if ever needed.
- **Demo video + social media post**: done.

Week 1 is fully complete. Ongoing work (Week 2's multimodal evidence agent and beyond) is tracked in
`../STATUS.md`.
...
- `LOADTEST.md` — load/stress test results (10 & 50 concurrent users) + baseline CPU/memory profile
- `OBSERVABILITY.md` — dashboard + alert definitions (p95 latency, error rate, tool failure rate, strip rate)
- `COST_ANALYSIS.md` — actual dev spend + projected cost at scale
- `PHI_AUDIT.md` — PHI redaction inventory + justification for Langfuse Cloud (no BAA) being acceptable
- `LANGFUSE_SELFHOST.md` — deferred future plan for full Langfuse self-hosting (Option A)
- `../../docs/seed-sample-patients.sql`, `../../docs/seed-additional-patients.sql` — sample patient data
- `../STATUS.md` — current project status (Week 2 onward)
