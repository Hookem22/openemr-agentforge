...
| `test_full_flow_integration.py` | The full pipeline chained in one turn -- upload -> vision extraction -> persistence -> supervisor routing -> guideline retrieval -> FHIR tool call -> a grounded, cited final answer -- through the real compiled graph (`run_turn`), everything external stubbed. Guards the wiring *between* stages that `test_ingestion_integration.py` and `test_rag_integration.py` each miss by only exercising their own stage in isolation; hand-verified to catch a disabled document/guideline citation check the same way the golden set's hard-gate rehearsal does. |
| `test_supervisor_worker_contract_unit.py` | The state *shape* handed between supervisor and workers -- `AgentState`'s exact field set, each worker node's precondition (raises if called without what it needs) and postcondition (what it guarantees is true of state afterward, on both the success and graceful-degradation paths). Distinct from `test_supervisor_unit.py` (routing decisions, not node state contracts) and `test_handoff_index_unit.py` (span tagging, not state shape). |
